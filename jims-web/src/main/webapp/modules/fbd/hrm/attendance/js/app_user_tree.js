...
var loadUserTree = function () {
    ...
    var loadUserPromise = $.get("/service/tool/find-user-tree?orgId=" + orgId, function (data) {
    ...
    loadUserPromise.done(function () {
        for (var i = 0; i < users.length; i++) {
            for (var j = 0; j < users.length; j++) {
                if (users[i].id == users[j].parent) {
                    if(users[j].type=='3'){
                        users[j].depId=users[i].id;
                        users[j].depName=users[i].name;
                    }
                    users[i].children.push(users[j]);
                }
            }
            if (users[i].children.length > 0 && !users[i].parent&&users[i].type!='3') {
                treeUsers.push(users[i]);
            }

            if (!users[i].parent && users[i].children <= 0&&users[i].type!='3') {
                treeUsers.push(users[i])
            }
        }
        $("#userTree").tree('loadData', treeUsers);
    })
}