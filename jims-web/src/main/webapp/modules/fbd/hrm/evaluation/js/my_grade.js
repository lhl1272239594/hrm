...
$(function () {
    ...
    $("#gradeGrid").datagrid({
        ...
        fit: true,//自动大小
        url: basePath + '/evaluationQuery/myGrade?orgId=' + orgId +'&month='+month+'&name='+name+'&typeId='+typeId,
        remoteSort: false,
        idField: 'id',
        singleSelect: true,//是否单选
        rownumbers: true,//行号
        columns: [[
            {field: 'typeName', title: '考评类型', width: '15%', align: 'center'},
            {field: 'name', title: '考评标题', width: '25%', align: 'center'},
    ...
    var searchData= function (page,rows){
        $("#gradeGrid").datagrid('reload', basePath + '/evaluationQuery/myGrade?orgId=' + orgId +'&month='+month+ '&page=' + page+ '&rows=' + rows+'&name='+name+'&typeId='+typeId);
        if(search){
            search=false;
    ...
    $("#grade").window({
        ...
        onOpen: function () {
            ...
            $("#objGrid").datagrid({
                ...
                rownumbers: true,//行号
                columns: [[
                    {field: 'typeName', title: '考核对象', width: '50%', align: 'center',
                        formatter:function (value,row,index) {
                            if(obj=="1"){
                                return row.deptName;
                            }
...
function detail(index) {
    ...
    row.endDate='';
    state=row.state;
    $.postJSON(basePath + "/evaluationQuery/getScore", JSON.stringify(row), function (data) {
...
function datagrid_reload(id) {
    $.get(basePath + '/manage/getDetail?id='+id+'&state='+state+ '&page=' + gradePage+ '&rows=' + gradeRows, function (data) {
