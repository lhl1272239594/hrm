...
$(function () {
    $("#personGrid").datagrid({
        ...
        columns: [[
            ...
            {field: 'userId', title: '名称', width: '100%', align: 'center',
    ...
    $("#userTree").tree({
        ...
        onCheck: function (node) {
            ...
            //var a=node;
            var c_nodes=''; //代存子节点
            var  p_nodes=$("#userTree").tree('getChecked');//所有选中节点
           /* if(p_nodes.length>0){
                //c_nodes.push(p_nodes);  //获取选中节点的子节点，并插入数组
                //a= c_nodes.name.join(",");//.join("");
                //alert(a);

            }*/
        for(var i=0;i<p_nodes.length;i++){
            c_nodes=c_nodes+'\''+p_nodes[i].id+'\'';
            if(i+1!=p_nodes.length){
                c_nodes+=',';
                }
            }
            var personName = $("#PERSON_NAME").textbox('getValue');
            ...
            /*if(node.parent==null)
             ...
             var url = basePath +'/salary-tmp/salary-list-todo1?orgId='+orgId+'&personName='+personName+'&deptId='+deptId+'&typeId='+typeId+'&deptIds='+deptIds;
             $("#dataGrid").datagrid("reload", url);
