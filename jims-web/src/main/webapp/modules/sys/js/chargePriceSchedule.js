function onloadMethod() {
    $('#list_data').datagrid({
        ...
        fit: true,//自动大小
        url:basePath+'/ChargePriceSchedule/list',
//        sortName: 'code',
//        sortOrder: 'desc',
        remoteSort: false,
        idField: 'fldId',
        ...
        columns: [[      //每个列具体内容
            ...
            {field: 'priceCoeffNumerator', title: '收费系数分子(住院)', width: '10%', align: 'center'},
            {field: 'priceCoeffDenominator', title: '收费系数分母(住院）', width: '18%', align: 'center'},
            {field: 'chargeSpecialIndicator', title: '适用特殊收适用特殊收费项目标志', width: '18%', align: 'center'},
            {field: 'numeratorOutp', title: '收费系数分子(门诊)', width: '18%', align: 'center'},
            {field: 'denominatorOutp', title: '收费系数分母(门诊)', width: '18%', align: 'center'},
        ...
        toolbar: [{
            ...
            iconCls: 'icon-add',
            handler: function () {
                $("#chargePriceScheduleForm").form('clear');
...
function del(id) {
    $.ajax({
        'type': 'POST',
        'url': basePath + '/ChargePriceSchedule/del',
 ...
 */
function saveDict(){
    $.postForm(basePath + '/ChargePriceSchedule/save', 'chargePriceScheduleForm', function (data) {
        if (data.data == 'success') {
...
function get(id) {
    ...
    $.ajax({
        'type': 'post',
        'url': basePath + '/ChargePriceSchedule/get',
        'contentType': 'application/json',
        'data': id = id,
        'dataType': 'json',
        'success': function (data) {
            $('#chargePriceScheduleForm').form('load', data);
        }
    });
}
